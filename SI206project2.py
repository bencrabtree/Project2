...
def grab_headlines():
    ...
    most_read = soup.find('h2', 'pane-title')
    lst = most_read.find_next('ol')

    headlines = []
    for headline in lst.find_all('a'):
...
def get_umsi_data():
    ...
    while (pages < 13):
        ...
        pages += 1
        url = fn + "&page=" + str(pages)
        data = requests.get(url, headers = {'User-Agent': 'SI_CLASS'})

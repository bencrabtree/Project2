...
def grab_headlines():
    #filename = open('opinion.html')
    url = 'http://www.michigandaily.com/section/opinion'
    r = requests.get(url)
    soup = BeautifulSoup(r.content, 'html.parser')
    most_read = soup.find('h2', 'pane-title') #class name of 'most read' header
    lst = most_read.find_next('ol') #finding ordered list that comes after 'most read' header

    headlines = []
    for headline in lst.find_all('a'): #get all the hyperlinks in the ordered list
        headlines.append(headline.string) #get the text headline that is wrapped in hyperlinks
    return headlines
...
def get_umsi_data():
    umsi_titles = {}
    fn = 'https://www.si.umich.edu/directory?field_person_firstname_value=&field_person_lastname_value=&rid=All'
    html = requests.get(fn, headers = {'User-Agent': 'SI_CLASS'})
    ...
    while (pages < 13):
        ...
        divs = soup.find_all('div', class_="field-item even") #this is the class we need to access

        for element in divs:
            if element.string != None: #if not empty
                temp.append(element.string)
        # temp is a list where every other element is the name of a person,
        # and every other element is their job title
        for i in range(len(temp)):
            if (i % 2) != 0: # if even
                names.append(temp[i]) #is a name
            else:
                titles.append(temp[i]) #is a title

        for index in range(len(titles)):
            umsi_titles[titles[index]] = names[index]

        pages += 1
        url = fn + "&page=" + str(pages) #each loop searches the next page
        html = requests.get(url, headers = {'User-Agent': 'SI_CLASS'})
        soup = BeautifulSoup(html.text, 'html.parser') #make new soup every time loop runs for new html page
